...
matrix ff5T(matrix x, matrix ud1, matrix ud2) {
    ...
    if (!isnan(ud1(0, 0))) {
        int* s = get_size(ud1);
        if (s[1] == 2) { // Determine if it is matrix A from Powell
             matrix p = get_col(ud1, 0);
             matrix d = get_col(ud1, 1);
             arg = p + m2d(x) * d;
        }
        delete[] s;
    }
    ...
    double f1 = a * (pow(arg(0) - 3.0, 2) + pow(arg(1) - 3.0, 2));
    
    double f2 = (1.0 / a) * (pow(arg(0) + 3.0, 2) + pow(arg(1) + 3.0, 2));
