...
void lab3()
{
	...
	Y0_sim(0) = 0.0;    // x0
	Y0_sim(1) = 100.0;  // y0
	Y0_sim(2) = optX.x(0);  // v0x (z optymalizacji)
	Y0_sim(3) = 0.0;    // v0y

	matrix params_sim(2, 1);
	params_sim(0) = optX.x(0);  // v0x
	params_sim(1) = optX.x(1);  // omega

	matrix *Y = solve_ode(df3, 0.0, 0.01, 7.0, Y0_sim, params_sim, NAN);  // t0=0, dt=0.01, t_end=7
